...
class VWOClient {
    ...
    async destroy() {
        ...
        try {
            ...
            if (BatchEventsQueue_1.BatchEventsQueue.Instance) {
                try {
                    await BatchEventsQueue_1.BatchEventsQueue.Instance.destroy();
                }
                catch (error) {
                    logger_1.LogManager.Instance.error('Error destroying BatchEventsQueue: ' + error);
                }
            }
            // Clear settings
            this.settings = null;
