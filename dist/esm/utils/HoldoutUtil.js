...
import { createHoldoutPayload } from './NetworkUtil.js';
import { EventEnum } from '../enums/EventEnum.js';
import { sendImpressionForVariationShown, sendImpressionForVariationShownInBatch } from './ImpressionUtil.js';
 ...
 */
export function getApplicableHoldouts(settings, featureId) {
 ...
 */
export async function getMatchedHoldouts(serviceContainer, feature, context, storedData) {
    ...
    const matchedHoldouts = [];
    const holdoutPayloads = [];
    ...
    for (const holdout of applicableHoldouts) {
        if (alreadyEvaluatedHoldoutIds.includes(holdout.getId())) {
            serviceContainer.getLogManager().debug(buildMessage(DebugLogMessagesEnum.HOLDOUT_SKIP_EVALUATION, {
        ...
        if (segments && Object.keys(segments).length > 0) {
            ...
            serviceContainer.getLogManager().info(buildMessage(InfoLogMessagesEnum.HOLDOUT_SEGMENTATION_SKIP, {
                holdoutId: holdout.getId(),
        ...
        if (!segmentPass) {
            serviceContainer.getLogManager().info(buildMessage(DebugLogMessagesEnum.HOLDOUT_SEGMENTATION_FAIL, {
                userId: context.getId(),
            ...
            if (isInHoldout) {
                serviceContainer.getLogManager().info(buildMessage(InfoLogMessagesEnum.HOLDOUT_SHOULD_EXCLUDE_USER, {
                    userId: context.getId(),
                    bucketValue: bucket,
                    holdoutGroupName: holdout.getName(),
                ...
                serviceContainer.getLogManager().debug(buildMessage(DebugLogMessagesEnum.HOLDOUT_SHOULD_NOT_EXCLUDE_USER, {
                    userId: context.getId(),
                    holdoutGroupName: holdout.getName(),
                    featureId: featureId,
                }));
 ...
 */
export async function sendNetworkCallsForNotInHoldouts(serviceContainer, feature, context, decision, storedData, storageService) {
    ...
    const updatedNotInHoldoutIds = [...(storedData?.notInHoldoutId ?? [])];
    const isInHoldoutIds = [...(storedData?.isInHoldoutId ?? [])];
    const batchPayload = [];
    ...
    if (updatedNotInHoldoutIds.length > initialNotInHoldoutCount) {
        new StorageDecorator().setDataInStorage({ featureKey: feature.getKey(), context, notInHoldoutId: updatedNotInHoldoutIds }, storageService, serviceContainer);
    }
