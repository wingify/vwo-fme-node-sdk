 ...
 */
export function getTrackingUsagePayloadData(serviceContainer, context) {
    const userId = context.getId();
    const properties = _getEventBasePayload(serviceContainer.getSettingsService(), userId, EventEnum.USER_EVALUATED, context.getUserAgent(), context.getIpAddress());
    if (context.getSessionId() !== 0) {
        properties.d.sessionId = context.getSessionId();
    }
    // if uuid is present in the context, use it, otherwise generate a new one
    ...
    serviceContainer.getLogManager().debug(buildMessage(DebugLogMessagesEnum.IMPRESSION_FOR_TRACKING_USAGE, {
        accountId: serviceContainer.getSettingsService().accountId.toString(),
        userId,
    }));
    return properties;
 ...
 */
export function createHoldoutPayload(serviceContainer, eventName, holdoutId, variationId, context, featureId) {
    const userId = context.getId();
    const properties = _getEventBasePayload(serviceContainer.getSettingsService(), userId, eventName, context?.getUserAgent(), context?.getIpAddress());
